...
import { getServiceDetails,editService } from "../../store/slices/category";
...
const save = (e) => {
    if (serviceName === "") {
      ...
      if(slots === ""){
        setSlotsError("please enter slots limit")
      }
...
.catch(({ message }) => {
    ...
    useEffect(() => {
        dispatch(getServiceDetails(item))
        .unwrap()
        .then((data) =>{
         setServiceName(data?.service?.data[0]?.name);
         setMedia(data?.service?.data[0]?.mediaLink);
         setPrice(data?.service?.data[0]?. pricePerHour);
         setGst(data?.service?.data[0]?. gstPercent);
         setPercent(data?.service?.data[0]?. platformCostPercent);
         setDate(data?.service?.data[0]?.createdAt.toString().split("T")[0]);
         setSlots(data?.service?.data[0]?.slotsLimit);
        })
        .catch(({ message }) => {
            alert(message);
          });
